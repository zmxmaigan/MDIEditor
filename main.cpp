...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpszCmdParam, int nCmdShow) {
	...
	g_hMainWindow = CreateWindowEx(WS_EX_APPWINDOW,g_szAppName,"MDIEditor增强版",WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case CM_WINDOW_ARRANGE:
					...
					break;
				case Githublink:{
					system("start https://zmxmaigan.github.io/MDIEditor/");
					break;
				} 
				default: {
			...
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_SAVEAS: {
					...
					if(GetFileName(hwnd, szFileName, TRUE)) {
						if(!SaveFile(GetDlgItem(hwnd, IDC_CHILD_EDIT), szFileName)) {
							MessageBox(hwnd, "无法正确的保存文件", "Error.",MB_OK | MB_ICONEXCLAMATION);
							return 0;
						} else {
